...
PROGRAM CLOUDPROGRAM
  ...
  INTEGER(KIND=JPIM) :: NGPBLKS,NUMOMP, NGPTOTG,ICEND
  INTEGER(KIND=JPIM),PARAMETER    :: NPROMA =128
   INTEGER(KIND=JPIM),PARAMETER    :: NGPTOT =1024


  CALL CLOUDSCOUTER(NBLOCKS,NGPBLKS,NUMOMP, NGPTOT, NGPTOTG,NPROMA,    KLON,    KLEV,&
  & PTSPHY,&
  & PT, PQ,&
...
SUBROUTINE CLOUDSCOUTER&
  !---input
  & (NBLOCKS,NGPBLKS,NUMOMP, NGPTOT, NGPTOTG,NPROMA,    KLON,    KLEV,&
  & PTSPHY,&
  & PT, PQ,&
  & tendency_cml_u,tendency_cml_v,tendency_cml_T,&
  & tendency_cml_o3,tendency_cml_q,tendency_cml_a,&
...
& RCL_CONST6R, RCL_FZRAB, RCL_FZRBB, LCLDEXTRA, LCLDBUDGET, NSSOPT, NCLDTOP,&
...
!USE YOMJFH   , ONLY : N_VMASS
  ...
  LOGICAL              :: LDMAINCALL       ! T if main call to cloudsc
  REAL(KIND=JPRB)      ::PEXTRA(KLON,KLEV,KFLDX,NBLOCKS) ! extra fields
  INTEGER(KIND=JPIM) :: JKGLO,IBL,ICEND,NGPBLKS,NUMOMP,NGPTOTG
...
& RCL_CONST2R, RCL_CONST3R, RCL_CONST4R, RCL_FAC1,RCL_FAC2, RCL_CONST5R,&
& RCL_CONST6R, RCL_FZRAB,  NSSOPT, NCLDTOP,&
&  LAERLIQAUTOLSP,&
...
INTEGER(KIND=JPIM) :: JKGLO,IBL,ICEND,NGPBLKS,NUMOMP, NGPTOT, NGPTOTG,NPROMA
...
DO JL=KIDIA,KFDIA
  ZTRPAUS(JL)=0.1
  ZPAPHD(JL)=1.0/PAPH(JL,KLEV+1)
ENDDO
DO JK=1,KLEV-1
  DO JL=KIDIA,KFDIA
    ZSIG=PAP(JL,JK)*ZPAPHD(JL)
    IF (ZSIG>0.1.AND.ZSIG<0.4.AND.ZTP1(JL,JK)>ZTP1(JL,JK+1)) THEN
      ZTRPAUS(JL)=ZSIG
    ENDIF
  ENDDO
ENDDO
