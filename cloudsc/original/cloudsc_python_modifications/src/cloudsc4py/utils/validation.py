...
def validate_storage_2d(name, src: Storage, trg: Storage) -> bool:
    src_np = to_numpy(src)
    trg_np = to_numpy(trg)
    mi = min(src_np.shape[0], trg_np.shape[0])
    mj = min(src_np.shape[1], trg_np.shape[1])
    print(f"Shape {name}", mi, mj)
    print("Min", np.min(src_np), np.min(trg_np))
    print("Max", np.max(src_np), np.max(trg_np))
    return np.allclose(src_np[:mi, :mj], trg_np[:mi, :mj], atol=1e-18, rtol=1e-12)


def validate_storage_3d(name, src: Storage, trg: Storage) -> bool:
    src_np = to_numpy(src)
    trg_np = to_numpy(trg)
    mi = min(src_np.shape[0], trg_np.shape[0])
    mj = min(src_np.shape[1], trg_np.shape[1])
    mk = min(src_np.shape[2], trg_np.shape[2])
    print(f"Shape {name}", mi, mj, mk)
    print("Min", np.min(src_np), np.min(trg_np))
    print("Max", np.max(src_np), np.max(trg_np))
    return np.allclose(src_np[:mi, :mj, :mk], trg_np[:mi, :mj, :mk], atol=1e-18, rtol=1e-12)


def validate_field(name, src: DataArray, trg: DataArray) -> bool:
    if src.ndim == 2:
        return validate_storage_2d(name, src.data, trg.data)
    elif src.ndim == 3:
        return validate_storage_3d(name, src.data, trg.data)
    else:
...
def validate(src: DataArrayDict, trg: DataArrayDict) -> Tuple[str]:
    return tuple(
        name
        for name in src
        if name in trg and name != "time" and not validate_field(name, src[name], trg[name])
    )
