...
for size in [8, 16, 32, 64, 128, 256]:
    ...
    cpu_filter_by_size = [df[df['size'] == asize] for df in cpu_frames]
    ...
    ax1.set_ylim(top=1500 * size // 8)
    ax1.set_xlabel('CPU sequential')
    ax2.set_ylabel('')
    ax2.set_ylim(top=90 * size // 8)
    ax2.set_xlabel('CPU parallel')
    ax3.set_ylabel('')
    ax3.set_ylim(top=30 * size // 8)
    ax3.set_xlabel('GPU')
    fig.tight_layout()
    plt.show()
    fig.savefig(f'fig10_{size}.pdf', bbox_inches='tight')
